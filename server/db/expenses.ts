...
const addRow = async (
	...
	const result = await pool.query(
        ...
        RETURNING id, name, value, expense_type, is_recurring, expense_date, recurring_day, recurring_interval, user_id`,
		[
			name,
			value,
			expense_type,
...
const getRows = async (userId: number) => {
	const result = await pool.query(
		`SELECT id, name, value, expense_type, is_recurring, TO_CHAR(expense_date, 'YYYY-MM-DD') as expense_date, recurring_day, recurring_interval FROM expenses
		WHERE user_id = $1`,
		[userId],
	);
	return result.rows;
