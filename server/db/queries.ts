...
const addRow = async (
	...
	const result = await pool.query(
        ...
        RETURNING id, name, value, expense_type ", is_recurring, expense_date, recurring_day, recurring_interval`,
		[
			name,
			value,
			expense_type,
...
const getRows = async () => {
	const result = await pool.query(
		`SELECT id, name, value, expense_type", is_recurring, expense_date, recurring_day, recurring_interval  FROM expenses`,
	);
	return result.rows;
};
...
const editRow = async (
	...
	const result = await pool.query(
		...
		[id, name, value, recurring_day, expense_type],
	);
	return result.rows[0];
};
